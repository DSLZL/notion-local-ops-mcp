...
var coreTools = []Tool{
	...
	{
		Name:        "tcp_connect",
		Description: "Open a persistent outbound TCP connection for interactive CTF-style workflows.",
		InputSchema: requiredObjectSchema([]string{"host", "port"},
			stringProperty("host", "Remote host or IP to connect to."),
			intProperty("port", "Remote TCP port."),
			intProperty("timeout_seconds", "Optional connection timeout in seconds."),
		),
	},
	{
		Name:        "tcp_send",
		Description: "Send text or base64-decoded bytes to an existing persistent TCP connection.",
		InputSchema: requiredObjectSchema([]string{"connection_id"},
			stringProperty("connection_id", "TCP connection id returned by tcp_connect."),
			stringProperty("text", "Optional plain-text payload to send."),
			stringProperty("content_base64", "Optional base64 payload for raw bytes."),
			boolProperty("append_newline", "When true, append a newline in text mode."),
		),
	},
	{
		Name:        "tcp_read",
		Description: "Read from a persistent TCP connection with timeout, max-bytes, and optional delimiter matching.",
		InputSchema: requiredObjectSchema([]string{"connection_id"},
			stringProperty("connection_id", "TCP connection id returned by tcp_connect."),
			intProperty("timeout_seconds", "Optional read timeout in seconds."),
			intProperty("max_bytes", "Optional maximum bytes to read in this call."),
			stringProperty("read_until", "Optional text delimiter that stops the read when matched."),
			stringProperty("read_until_base64", "Optional base64-encoded byte delimiter that stops the read when matched."),
			stringProperty("output_mode", "Optional output mode: text or base64."),
		),
	},
	{
		Name:        "tcp_close",
		Description: "Close a persistent TCP connection and mark it inactive.",
		InputSchema: requiredObjectSchema([]string{"connection_id"},
			stringProperty("connection_id", "TCP connection id returned by tcp_connect."),
		),
	},
